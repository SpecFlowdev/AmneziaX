...
Всё, что отдаёт нода, отдаёт **xray-core**: на ноде один процесс, а установка
требует одного бинарника.
...
HTTPUpgrade, XHTTP. Профиль **и есть** документ xray, а не форма, которую панель
во что-то переводит.
...
**Чего нет и почему.** OpenVPN и Cloak. OpenVPN и Cloak — ещё дальше: OpenVPN тянет за собой
TUN-интерфейс и собственный удостоверяющий центр, а Cloak — слой обфускации
перед прокси со своим ключевым материалом и своим списком пользователей. Ни то,
ни другое не укладывается в «профиль — это один документ», и здесь они не
заявлены.
