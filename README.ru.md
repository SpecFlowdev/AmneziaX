...
| **Профили конфигурации** | Полноценные документы Xray, hysteria2 или sing-box: редактируются как JSON и проверяются правилами того ядра, которое их запустит, до того, как попадут на сервер. Выберите ядро — панель подставит готовый рабочий документ. |
...
| **Импорт одним нажатием** | На странице подписчика сразу его приложения — Happ, v2rayNG, Streisand, Hiddify, Shadowrocket, sing-box, Clash, V2Box — каждое своей URL-схемой со ссылкой внутри. Первыми идут те, что работают на устройстве, с которого страницу открыли, а кнопка копирования остаётся как способ, который срабатывает всегда. |
| **Командная палитра** | `Ctrl`/`⌘` + `K` из любого места: любая страница в пару букв, и тем же полем находится пользователь, нода, хост или сквад. Один запрос по всем четырём, а не четыре обращения на каждое нажатие клавиши. |
| **Массовое создание пользователей** | Класс, офис или клиентов реселлера — за один раз, по префиксу с количеством или списком из буфера. Номера дополняются нулями, чтобы имена сортировались так же, как читаются; список показывается до создания, а дубликат стоит одного имени, а не всей партии. Имена и ссылки выгружаются файлом. |
| **Выгрузка в CSV** | Список пользователей с учётом фильтров — в формате, который уже понимают биллинг и таблицы. |
| **Предупреждения до отключения** | Срок действия и лимит трафика одинаково отключают подписчика по расписанию, и раньше это было видно только постфактум. Теперь о каждом предупреждаем заранее — событиями, на которые можно подписать вебхук или Telegram. Отправляется по одному разу, а не раз в минуту: пометка ставится тем же запросом, который читает строку, а продление или месячный сброс перевзводят её. |
| **Панель «требует внимания»** | Одна карточка на дашборде: ноды офлайн, с проблемами или сверх лимита; подписчики отключённые, истекающие или у предела; платежи за ноды на ближайшую неделю — с именами, а не только числами. Когда всё в порядке, она умещается в одну строку. |
| **WireGuard** | Отдаётся самим xray-core: ноде не нужен второй бинарник и переустановка. У каждого подписчика своя пара ключей Curve25519 и постоянный адрес внутри туннеля; на ноду уходят только публичные ключи, а готовый `.conf` приходит по той же ссылке подписки, что и всё остальное. |
...
Нода отдаёт их одним из трёх ядер, и какое именно — говорит профиль:
**xray-core**, **hysteria2** или **sing-box**. Большинству парков хватает
первого, поэтому обычная нода держит один процесс и ставит один бинарник.
...
| **Trojan** | |
| **Shadowsocks** | |
| **WireGuard** | своя пара ключей Curve25519 и постоянный адрес в туннеле у каждого подписчика |
| **Hysteria2** | через sing-box или собственное ядро, ставится рядом с xray-core |
| **TUIC** | через sing-box |

Транспорты — те, что указаны в самом документе профиля: TCP, WebSocket, gRPC,
HTTPUpgrade, XHTTP. Профиль **и есть** документ своего ядра, а не форма, которую
панель во что-то переводит. Поэтому выбор ядра — первое поле в редакторе
профиля: документ проверяет, рендерит и запускает именно то ядро, которое вы
назвали, а панель предлагает готовый рабочий документ для каждого.

Доступ от ядра не зависит. Сквад выдаёт инбаунд, хост его публикует, у
пользователя кончился трафик — и он пропадает из рабочей конфигурации:
одинаково, какое бы из трёх ядер за этим ни стояло.

Hysteria2 — не протокол xray (на него xray отвечает `unknown config id`),
поэтому нода с ним держит второй бинарник рядом с xray-core; его ставит
установщик ноды. Чтобы добавить его на уже работающую ноду, установщик нужно
запустить заново.

**Чего нет и почему.** OpenVPN и Cloak. OpenVPN тянет за собой TUN-интерфейс и
собственный удостоверяющий центр — сертификат на каждого подписчика нужно
выпустить, хранить и уметь отозвать, — а Cloak — это слой обфускации перед
прокси со своим ключевым материалом и своим списком пользователей. Ни то, ни
другое не сводится к смене ядра под документом, поэтому здесь они не заявлены.
...
| Профили конфигурации | Выбор ядра |
|---|---|
| <img src="docs/assets/profiles.png" alt="Профили для трёх разных ядер" width="100%"> | <img src="docs/assets/profile-editor.png" alt="Селектор ядра и готовый документ" width="100%"> |
...
| Командная палитра | Страница подписчика |
|---|---|
| <img src="docs/assets/palette.png" alt="Ctrl+K находит пользователя" width="100%"> | <img src="docs/assets/import.png" alt="Импорт в приложения одним нажатием" width="100%"> |

| Что требует внимания | |
|---|---|
| <img src="docs/assets/attention.png" alt="Панель «требует внимания» на дашборде" width="100%"> | |
...
подписок. Профиль называет ядро, которое его запустит, а редактор сразу
подставляет рабочий документ для этого ядра. Инспекторы отвечают, кто и чем
забирал подписку и кто сейчас гонит
