...
**Чего нет и почему.** Hysteria2 и TUIC — не протоколы xray: на оба он отвечает
`unknown config id`. Отдавать их — значит держать на ноде второй бинарник, иметь
агента, который ведёт больше одного процесса, и второй путь сбора статистики.
Это изменение на стороне ноды, то есть оно не появится без переустановки агента
на уже работающих серверах. OpenVPN и Cloak — ещё дальше: OpenVPN тянет за собой
