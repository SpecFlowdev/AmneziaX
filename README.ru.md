...
| **HTTPS сразу** | Caddy терминирует TLS и для панели, и для управляющего потока нод — наружу открыты только 80 и 443. |
...
На сервере панели открыто всего два порта:

| Порт | Кто подключается | Комментарий |
|---|---|---|
| `80` | Let's Encrypt и браузеры при редиректе | Нужен для выпуска и продления сертификата. |
| `443` | Администраторы, подписчики **и агенты нод** | Caddy отдаёт веб-интерфейс, API и gRPC-поток нод на одном порту. |
...
### Как поток нод делит порт 443

gRPC-вызов к сервису ноды приходит как `POST /node.v1.NodeControl/Connect`,
поэтому Caddy ловит этот путь и отправляет его на gRPC-слушатель панели по h2c,
а всё остальное — в веб-интерфейс. Агенты подключаются к `ваш-домен:443` по
обычному TLS: лишний порт открывать не нужно, а трафик нод шифруется тем же
сертификатом, что и панель.
...
| `PANEL_GRPC_PUBLIC_PORT` | `9090` | Порт, к которому подключается агент ноды. За Caddy — `443`. |
...
```
   администратор ─┐
   подписчик    ─┤  https://ваш-домен        ┌──────────────────────┐
   агент ноды   ─┘         :443       ──────▶│  Caddy  (TLS, ACME)  │
                                             └──────────┬───────────┘
                           /node.v1.NodeControl/* ──────┤────── всё остальное
                                             ┌──────────▼───────────┐
